  ...
  if (!fs.existsSync(path.join(project.path, 'package.json'))) {
    ...
    if (project.isProject) {
      ...
      try {
        ...
        logger.info(`Could not get flow file content : ${errorFileContent}`)
      }
    } else {
...
function getCfgFile(projectPath, cfgFilename) {
  if (!fs.existsSync(path.join(projectPath, cfgFilename))) {
    logger.debug(`Missing node-red-flow-splitter config file '${cfgFilename}' at root of the project ${projectPath}`);
    return null
  }
  else {
    try {
      ...
      logger.error(`Failed returning/parsing node-red-flow-splitter config file '${cfgFilename}' : ${error}`);
      return null
    }
  }
}
