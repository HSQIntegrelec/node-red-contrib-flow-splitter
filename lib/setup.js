...
function getFlowFile(project) {
  ...
  if (!fs.existsSync(path.join(project.path, 'package.json'))) {
    ...
    if (project.isProject) {
      ...
      try {
        if (fs.existsSync(path.join(project.path, projectPackageJson["node-red"].settings.flowFile))) {
          fileContent = JSON.parse(fs.readFileSync(path.join(project.path, projectPackageJson["node-red"].settings.flowFile)))
        }
        else {
          logger.debug(`Flow file does not exist '${path.join(project.path, projectPackageJson["node-red"].settings.flowFile)}'`)
        }
